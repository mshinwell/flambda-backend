(* Generator for large OCaml modules to test cross-partition calls.
   ...
   This generates modules that are each larger than stdlib.a (~1.5MB) to ensure
   they end up in different partitions when using the dissector. *)

let num_functions =
  10000 (* Number of functions per module - keep under stack limit *)

let array_size =
  130000 (* Size of array literals - larger to exceed stdlib size *)
...
let generate_module_b () =
  ...
  Buffer.add_string buf
    "let call_a_increment () = Large_module_a.increment ()\n";
...
let generate_module_c () =
  ...
  Buffer.add_string buf
    "let call_a_increment () = Large_module_a.increment ()\n";
  Buffer.add_string buf
    "let call_b_increment () = Large_module_b.increment ()\n";
  Buffer.add_string buf "let call_a_magic () = Large_module_a.get_magic ()\n";
  Buffer.add_string buf
    "let call_both_functions x = Large_module_a.call_all x + \
     Large_module_b.call_all x\n";
  Buffer.add_string buf "let get_a_arr_element i = Large_module_a.arr.(i)\n";
