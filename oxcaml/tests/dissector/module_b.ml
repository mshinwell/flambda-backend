...
let get_magic_doubled () = Module_a.get_magic () * 2

(* Use Module A's types and functions *)
let point_at x y : Module_a.point = { x; y }

let distance_from_origin p = Module_a.compute_distance p Module_a.origin

(* More complex cross-module interaction *)
let factorial_sum n =
  let rec loop acc i =
    if i > n then acc else loop (acc + Module_a.factorial i) (i + 1)
  in
  loop 0 1

(* Access Module A's array *)
let double_array_sum () = 2 * Module_a.sum_array Module_a.sample_array

(* String operations using Module A *)
let greet_world () = Module_a.make_greeting "World"
