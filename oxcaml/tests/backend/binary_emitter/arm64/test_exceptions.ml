...
let check_positive n =
  if n < 0 then raise (My_error "negative value")
  else n
...
let nested_try x =
  try
    try
      if x < 0 then raise Not_found_custom
      else if x = 0 then raise (With_value 0)
      else x * 2
...
let () =
  ...
  let _ = (try find_first (fun x -> x > 3) [1; 2; 3; 4; 5] with Not_found -> 0) in
