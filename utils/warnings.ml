...
(*| Deprecated --> alert "deprecated" *)    (*  3 *)
  ...
  | Unnecessarily_partial_tuple_pattern_oxcaml (* 189 *)
...
let descriptions = [
  ...
  { number = 188;
    ...
    since = since 5 1 };
  { number = 189;
    names = ["unnecessarily-partial-tuple-pattern-oxcaml"];
    description = "A tuple pattern ends in .. but fully matches its expected \
                   type.";
    since = since 5 1 };
  { number = 190;
...
let message = function
  ...
  | Unnecessarily_partial_tuple_pattern ->
      msg
        ...
        Style.inline_code ".."
  | Unmutated_mutable v ->
      msg "mutable variable %a was never mutated."
        Style.inline_code v
  | Incompatible_with_upstream (Immediate_erasure id) ->
      msg
        "Usage of layout immediate/immediate64 in %a@ \
  ...
  | Incompatible_with_upstream (Non_value_sort layout) ->
      msg
        "External declaration here is not upstream compatible.@ \
         ...
         %a encountered."
        Style.inline_code layout
  | Incompatible_with_upstream (Unboxed_attribute layout) ->
      msg
        "%a attribute must be added to external declaration@ \
  ...
  | Unnecessarily_partial_tuple_pattern_oxcaml ->
      msg
        "This tuple pattern@ unnecessarily@ ends in %a,@ as@ it@ explicitly@ \
         matches@ all@ components@ of@ its@ expected@ type."
        Style.inline_code ".."
  | Probe_name_too_long name ->
      msg
        "This probe name is too long: %a.@ \
         Probe names must be at most 100 characters long."
        Style.inline_code name
  | Zero_alloc_all_hidden_arrow s ->
      msg
        "The type of this item is an@ alias of a function type, but the@ \
         %a attribute for@ this signature does not apply to it because@ \
         its type is not@ syntactically a function type. If it should be@ \
         checked, use an@ explicit zero_alloc attribute with an arity.@ \
         If not, use an explicit@ zero_alloc ignore attribute."
        Style.inline_code ("[@@@zero_alloc " ^ s ^ "]")
  ...
  | Unboxing_impossible ->
      msg
        "This %a attribute cannot be used.@ \
         The type of this value does not allow unboxing."
        Style.inline_code "[@unboxed]"
  | Mod_by_top modifier ->
      msg
        "%a is the top-most modifier.@ \
         Modifying by a top element is a no-op."
        Style.inline_code modifier
  | Modal_axis_specified_twice {axis; overriden_by} ->
      msg
        "This %a is overriden by %a later."
        Style.inline_code axis
        Style.inline_code overriden_by
  | Atomic_float_record_boxed ->
      msg
        "This record contains atomic@ float fields, which prevents@ \
         the float record optimization. The@ fields of this record will@ \
         be boxed instead of being@ represented as a flat float array."
;;
