...
let jit_load (type a r)
  ...
  in
  (* Bind the symbols the compiler aliased to runtime atoms (zero-sized
     statics of unloadable units; see
     [Cmm_helpers.register_unloadable_atom_aliased_symbol]): each such symbol
     resolves to the *value* of the runtime's atom of the recorded tag, i.e.
     one word past the exported [caml_atom_<tag>] header symbol (this is
     [caml_atom (tag)] on the C side). They are added to [local_symbols] so
     that both this unit's relocations and later units' references resolve to
     the permanent atom. *)
  let local_symbols =
    List.fold_left atom_aliases ~init:local_symbols
      ~f:(fun syms (name, tag) ->
        ...
        | Some addr -> Symbols.add syms name (Address.add_int addr 8)
        | None ->
...
let jit_callback ~phrase_name ~outcome_ref packed =
  ...
  in
  let atom_aliases = Cmm_helpers.flush_unloadable_atom_aliased_symbols () in
...
let jit_load_lambda ~phrase_name ppf (program : Lambda.program) =
  ...
  let restore () =
    ...
    ignore
      (Cmm_helpers.flush_unloadable_atom_aliased_symbols ()
        : (string * int) list)
  in
  (match
