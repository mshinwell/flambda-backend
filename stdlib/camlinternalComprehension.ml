...
let rec rev_dlist_concat_map l f acc = (* [List.fold_left (fun acc el -> f el acc) acc l], but more [local_] *)
  match l with
  | []      -> acc
  | el :: l -> rev_dlist_concat_map l f (f el acc)
;;

let rec rev_dlist_concat_iterate_up from to_ f acc = if to_ < from
  then acc
  else rev_dlist_concat_iterate_up (from + 1) to_ f (f from acc)
;;

let rec rev_dlist_concat_iterate_down from to_ f acc = if to_ > from
  then acc
