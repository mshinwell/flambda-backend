...
(* Reset the environment and record the name of the unit being compiled
   (including any associated -for-pack prefix). *)
...
(* Means of getting the export info found in the .cmx file of the given unit. *)
...
val need_send_fun :
  Cmm.machtype list -> Cmm.machtype -> Cmx_format.alloc_mode -> unit
(* Record the need of a currying (resp. application, message sending) function
...
(* Enter the given infos in the cache. The infos will be honored by
   [symbol_for_global] and [global_approx] without looking at the corresponding
   .cmx file. *)

val require_global : Compilation_unit.t -> unit
(* Enforce a link dependency of the current compilation unit to the required
   module *)
...
(* CR mshinwell: see comment in .ml val
   ensure_sharing_between_cmi_and_cmx_imports : (_ * (Compilation_unit.t * _)
   option) list -> (Compilation_unit.t * 'a) list -> (Compilation_unit.t * 'a)
   list *)
