...
let value_kind vk = Format.asprintf "%a" value_kind vk
...
let rec structured_constant ppf = function
  ...
  | Uconst_block (tag, l) ->
    ...
    fprintf ppf ")"
  | Uconst_float_array [] -> fprintf ppf "floatarray()"
  ...
  | Uconst_closure (clos, sym, fv) ->
    let funs ppf = List.iter (fprintf ppf "@ %a" one_fun) in
    let sconsts ppf scl =
...
and one_fun ppf f =
  let idents ppf =
    List.iter (fun (x, k) ->
        fprintf ppf "@ %a%a" VP.print x Printlambda.value_kind k)
  in
  fprintf ppf "(fun@ %s%s@ %d@ @[<2>%a@]@ @[<2>%a@])" f.label
    (value_kind f.return) (snd f.arity) idents f.params lam f.body
...
and phantom_defining_expr ppf = function
  ...
  | Uphantom_block { tag; fields } ->
    ...
    List.iter
      (fun field -> Format.fprintf ppf "%a; " Backend_var.print field)
      fields;
    Format.fprintf ppf "]"
...
and uconstant ppf = function
  | Uconst_ref (s, Some c) -> fprintf ppf "%S=%a" s structured_constant c
...
and lam ppf = function
  ...
  | Uclosure (clos, fv) ->
    let funs ppf = List.iter (fprintf ppf "@ @[<2>%a@]" one_fun) in
    let lams ppf = List.iter (fprintf ppf "@ %a" lam) in
  ...
  | Ulet (mut, kind, id, arg, body) ->
    let rec letbody ul =
      match ul with
      | Ulet (mut, kind, id, arg, body) ->
        fprintf ppf "@ @[<2>%a%s%s@ %a@]" VP.print id (mutable_flag mut)
          (value_kind kind) lam arg;
        letbody body
      | _ -> ul
    in
    fprintf ppf "@[<2>(let@ @[<hv 1>(@[<2>%a%s%s@ %a@]" VP.print id
  ...
  | Uphantom_let (id, defining_expr, body) ->
    let rec letbody ul =
      match ul with
      | Uphantom_let (id, defining_expr, body) ->
        fprintf ppf "@ @[<2>%a@ %a@]" Backend_var.With_provenance.print id
          phantom_defining_expr_opt defining_expr;
        letbody body
      | _ -> ul
    in
    fprintf ppf "@[<2>(phantom_let@ @[<hv 1>(@[<2>%a@ %a@]"
      Backend_var.With_provenance.print id phantom_defining_expr_opt
      defining_expr;
    let expr = letbody body in
    fprintf ppf ")@]@ %a)@]" lam expr
    ...
    let bindings ppf id_arg_list =
      ...
      List.iter
        (fun (id, l) ->
          ...
          fprintf ppf "@[<2>%a@ %a@]" VP.print id lam l)
        id_arg_list
    in
  ...
  | Uprim (prim, largs, _) ->
    let lams ppf largs = List.iter (fun l -> fprintf ppf "@ %a" lam l) largs in
    fprintf ppf "@[<2>(%a%a)@]" Printclambda_primitives.primitive prim lams
      largs
  | Uswitch (larg, sw, _dbg, _kind) ->
    ...
    let print_cases tag index cases ppf =
      for i = 0 to Array.length cases - 1 do
        fprintf ppf "@ @[<2>%t@ %a@]" (print_case tag index i) sequence
          cases.(i)
      done
    in
    let switch ppf sw =
      print_cases "int" sw.us_index_consts sw.us_actions_consts ppf;
      print_cases "tag" sw.us_index_blocks sw.us_actions_blocks ppf
    in
    fprintf ppf "@[<v 0>@[<2>(switch@ %a@ @]%a)@]" lam larg switch sw
  | Ustringswitch (larg, sw, d, _kind) ->
    let switch ppf sw =
      ...
      | None -> ()
    in
    fprintf ppf "@[<1>(switch %a@ @[<v 0>%a@])@]" lam larg switch sw
  | Ustaticfail (i, ls) ->
    let lams ppf largs = List.iter (fun l -> fprintf ppf "@ %a" lam l) largs in
  ...
  | Utrywith (lbody, param, lhandler, _kind) ->
    fprintf ppf "@[<2>(try@ %a@;<1 -1>with %a@ %a)@]" lam lbody VP.print param
      lam lhandler
  | Uifthenelse (lcond, lif, lelse, _kind) ->
    fprintf ppf "@[<2>(if@ %a@ %a@ %a)@]" lam lcond lam lif lam lelse
  | Usequence (l1, l2) -> fprintf ppf "@[<2>(seq@ %a@ %a)@]" lam l1 sequence l2
  | Uwhile (lcond, lbody) ->
    fprintf ppf "@[<2>(while@ %a@ %a)@]" lam lcond lam lbody
  | Ufor (param, lo, hi, dir, body) ->
    fprintf ppf "@[<2>(for %a@ %a@ %s@ %a@ %a)@]" VP.print param lam lo
  ...
  | Usend (k, met, obj, largs, (pos, _), _) ->
    ...
    let kind =
      if k = Lambda.Self
      then "self"
      else if k = Lambda.Cached
      then "cache"
      else ""
    in
    fprintf ppf "@[<2>(%s%s@ %a@ %a%a)@]" form kind lam obj lam met args largs
  | Uunreachable -> fprintf ppf "unreachable"
  | Uregion e -> fprintf ppf "@[<2>(region@ %a)@]" lam e
  | Utail e -> fprintf ppf "@[<2>(tail@ %a)@]" lam e

and sequence ppf ulam =
  match ulam with
  | Usequence (l1, l2) -> fprintf ppf "%a@ %a" sequence l1 sequence l2
  | _ -> lam ppf ulam

let clambda ppf ulam = fprintf ppf "%a@." lam ulam
...
let rec approx ppf = function
  | Value_closure (_, fundesc, a) ->
    ...
    if fundesc.fun_closed then Format.fprintf ppf "@ (closed)";
    if fundesc.fun_inline <> None then Format.fprintf ppf "@ (inline)";
  ...
  | Value_unknown -> Format.fprintf ppf "_"
  | Value_const c -> fprintf ppf "@[const(%a)@]" uconstant c
  | Value_global_field (s, i) -> fprintf ppf "@[global(%s,%i)@]" s i
