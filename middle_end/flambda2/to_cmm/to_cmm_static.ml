...
(* This one is nasty because with the OCaml 5 runtime, we have to use
   [caml_initialize] to fill up an inconstant statically-allocated block. So we
   should only be doing 64-bit writes! *)
let rec static_unboxed_int32_array_updates symb env res acc ~index_64bit
    (fields : _ Or_variable.t list) =
  ...
  | [sv] -> (
    ...
    | Const _ -> env, res, acc
    | Var (var, dbg) ->
      (* Here and elsewhere, the sign extensions are important, since 64-bit
         instructions are used to read the 32-bit quantities. *)
      C.make_update env res dbg Word_int ~symbol:(C.symbol ~dbg symb) var
        ~index:index_64bit ~prev_updates:acc)
  | Const _ :: Const _ :: r ->
    static_unboxed_int32_array_updates symb env res acc
      ~index_64bit:(index_64bit + 1) r
  | ((Const _ | Var _) as sv1) :: sv2 :: r -> (
    let dbg = Debuginfo.none in
    let make_const (sv : _ Or_variable.t) =
      match sv with
      | Const i32 -> C.int32 ~dbg i32
      | Var (_, dbg) -> C.int ~dbg 0
    in
    let const1 = make_const sv1 in let const2 = make_const sv2 in
    let  const =
C.or_int
(C.lsl_int const1 (C.int_const dbg 32) dbg)
const2
  in
  let var =
  ...
  in
    ...
    match (sv : _ Or_variable.t) with
    | Const _ -> static_unboxed_int32_array_updates symb env res acc (i + 1) r
    | Var (var, dbg) ->
      let env, res, acc =
        C.make_update env res dbg Word_int ~symbol:(C.symbol ~dbg symb) var
          ~index:index_64bit ~prev_updates:acc
