...
let machtype_of_kind (k : Flambda_kind.t) =
  ...
  | Naked_number (Naked_immediate | Naked_int32 | Naked_nativeint) ->
    Cmm.typ_int
  | Region -> (* See comments elsewhere about these bindings. *) Cmm.typ_int
...
let split_complex_binding ~env ~res (binding : complex binding) =
  ...
  | Splittable_prim { dbg; prim; args } ->
    let new_bindings, new_cmm_args =
      let args =
        List.map2
          (fun (arg, ece) kind -> arg, ece, machtype_of_kind kind)
          args
    ...
    let split_binding =
      { order = binding.order;
        ...
        machtype = P.Without_args.result_kind prim |> machtype_of_kind
      }
    in
