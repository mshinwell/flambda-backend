...
let translate_apply0 env res apply =
  ...
  | Call_kind.C_call { alloc; return_arity; param_arity; is_c_builtin } ->
    ...
    in
    let ty_args =
      List.map Env.exttype_of_kind (Flambda_arity.to_list param_arity)
    in
    ( wrap dbg
...
and let_prim env res ~num_normal_occurrences_of_bound_vars v p dbg body =
  ...
  in
  let result_machtype =
    Flambda_primitive.result_kind' p |> To_cmm_env.machtype_of_kind
  in
  let inline =
        ...
        let dummy_value =
          ...
          | Region | Rec_info ->
            Misc.fatal_errorf "No dummy value available for kind %a" K.print
              kind
        in
        C.letin_mut mut_var (To_cmm_env.machtype_of_kind kind) dummy_value cmm)
      trywith mut_vars
  in
  cmm, res
