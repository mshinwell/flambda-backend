...
(* Symbol identifying the module block at [path] within the current compilation
   ...
   for the actual module block, so that the forward-declaration [let symbol =
   ...] emitted by [Pannounce_module_block] and the eventual definition emitted
   by [close_program] share a single linkage name. For submodules and other
   paths we synthesize a unique linkage name with the path mangled in. *)
...
(* [Pinit_module_block (tag, _, _, _, path)] is translated directly into a
   ...
   Returns the updated accumulator and a [Bound_pattern.static] + [Named.t] that
   the caller can use to build the [Let_with_acc] expression. *)
let init_module_block_static_const ~tag ~field_simples =
  Static_const.block
...
let emit_init_module_block_let_symbol acc (path : Path.t) (tag : int)
  ...
  in
  let static_const = init_module_block_static_const ~tag ~field_simples in
  let approxs =
    args |> List.flatten
  ...
  let named =
    Flambda.Named.create_static_consts
      (Flambda.Static_const_group.create
         [ Flambda.Static_const_or_code.create_static_const ~forward_decl:false
             static_const ])
  in
  let acc, body_expr =
...
let convert_and_bind acc ~big_endian exn_cont ~register_const0
  ...
  | Pannounce_module_block (tag, _shape, _mode, path) ->
    let acc = declare_module_block_announce_symbol acc path tag in
