...
(* CR pchambart: define a record `target_config` to hold things like
   `big_endian` *)
let lambda_to_flambda ~mode ~symbol_for_global ~big_endian ~cmx_loader ~module_ident
  ...
  in
  CC.close_program ~mode ~symbol_for_global ~big_endian ~cmx_loader ~module_ident
