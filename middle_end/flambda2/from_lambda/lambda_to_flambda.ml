            ...
            (fun acc env ccenv stop_var _arity ->
              ...
              let close_if acc ccenv ~cond ~if_true ~if_false =
                ...
                let switch : IR.switch =
                  { numconsts = 2;
                    consts =
                      [0, false_cont, dbg, None, []; 1, true_cont, dbg, None, []];
                    failaction = None
                  }
                in
                CC.close_let_cont acc ccenv ~name:false_cont
                  ~is_exn_handler:false ~params:[] ~recursive:Nonrecursive
                  ~body:(fun acc ccenv ->
                    CC.close_let_cont acc ccenv ~name:true_cont
                      ~is_exn_handler:false ~params:[] ~recursive:Nonrecursive
                      ~body:(fun acc ccenv ->
