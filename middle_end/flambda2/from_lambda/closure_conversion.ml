...
(* [Pinit_module_block] is translated directly into a [Let_with_acc]
   [let_symbol] binding the module symbol (see
   [Lambda_to_flambda_primitives.emit_init_module_block_let_symbol]). As a
   result, the program ends with [apply_cont prog_return_cont [Simple.symbol
   module_symbol]] and the auxiliary continuation handler used by the previous
   [wrap_final_module_block] (which loaded fields from a runtime block and
   rebuilt the module symbol) is no longer needed. We just run [program] and let
   [prog_return_cont] act as the toplevel return continuation of the resulting
   [Flambda_unit]. *)
...
let close_program (type mode) ~(mode : mode Flambda_features.mode)
  ...
  in
  let _ = module_repr in
  (* The program now ends with [apply_cont prog_return_cont [Simple.symbol
     module_symbol]] (because [Pinit_module_block] is translated directly into a
     [let_symbol] in [Lambda_to_flambda_primitives]). We use [prog_return_cont]
     directly as the [Flambda_unit]'s return continuation; there is no need to
     wrap with a forwarder. *)
