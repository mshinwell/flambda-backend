...
let close_primitive acc env ~let_bound_ids_with_kinds named
  ...
  | ( ( Pmakeblock _ | Pinit_module_block _ | Pmakefloatblock _
    ...
    let acc, sym =
      ...
      | Pscalar _ | Pphys_equal _ | Pcpu_relax | Pannounce_module_block _ ->
...
(* Legacy module-block consolidation pass. The program ends by passing the
   dynamically-built module block to [prog_return_cont]; this function wraps it
   in a let-cont handler that re-binds the block as a static [let_symbol] for
   [module_symbol] and applies [return_cont] with that symbol. Used when the
   [-incremental-module-blocks] flag is off and [Translmod] therefore emits the
   legacy [Pmakeblock]-based module block construction. *)
...
(* When the [-incremental-module-blocks] flag is enabled, [Pinit_module_block]
   is translated directly into a [Let_with_acc] [let_symbol] binding the module
   symbol (see [Lambda_to_flambda_primitives.emit_init_module_block_let_symbol])
   and the program ends with [apply_cont prog_return_cont [Simple.symbol
   module_symbol]]. In that mode we don't need [wrap_final_module_block] --
   [prog_return_cont] is used directly as the [Flambda_unit]'s return
   continuation. *)
...
let close_program (type mode) ~(mode : mode Flambda_features.mode)
  ...
  let return_cont, (acc, body) =
    if !Clflags.incremental_module_blocks
    then
      (* The program already wraps the module block in a static [let_symbol] and
         returns the symbol via [prog_return_cont]. *)
      let acc, body = program acc env in
      let acc, body = bind_static_consts_and_code acc body in
      prog_return_cont, (acc, body)
    else
      (* Legacy path: the program returns a runtime block via
         [prog_return_cont]; [wrap_final_module_block] turns that into a static
         module-symbol binding and forwards to [return_cont]. *)
