...
let prepare_cmx_from_approx ~machine_width ~approxs ~module_symbol ~exported_offsets
    ...
    let create_typing_env reachable_names =
      let approxs =
        Symbol.Map.filter
          ...
          approxs
      in
      TE.Serializable.create_from_closure_conversion_approx ~machine_width approxs
    in
    let free_names_of_name name =
