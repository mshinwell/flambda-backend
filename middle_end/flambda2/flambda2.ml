...
let lambda_to_flambda ~ppf_dump:ppf ~prefixname ~machine_width (program : Lambda.program) =
  ...
  let { Closure_conversion.unit = raw_flambda;
    ...
    Profile.record_call "lambda_to_flambda" (fun () ->
        Lambda_to_flambda.lambda_to_flambda ~mode ~machine_width ~big_endian:Arch.big_endian
    ...
    | Normal, Normal ->
      ...
      let { Simplify.free_names;
        ...
        Profile.record_call ~accumulate:true "simplify" (fun () ->
            Simplify.run ~cmx_loader ~machine_width ~round ~code_slot_offsets raw_flambda)
      in
      (if Flambda_features.inlining_report ()
      then
