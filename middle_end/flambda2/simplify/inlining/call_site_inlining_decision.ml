...
let make_decision dacc ~simplify_expr ~function_type ~apply ~return_arity :
  ...
  let decision : Call_site_inlining_decision_type.t =
    ...
    | Default_inlined | Unroll _ | Always_inlined _ | Hint_inlined -> (
      ...
      then Missing_code
        ...
        (* The unrolling process is rather subtle, but it boils down to two
           steps:
           ...
           depth is [succ my_depth]). At that point, we short-circuit most of
           the inlining logic and inline if and only if n > 0.
        ...
        | None -> (
          (* lmaurer: This seems semantically dodgy: If we really think of a
             free depth variable as [Unknown], then we shouldn't be considering
             inlining here, because we don't _know_ that we're not unrolling.
             The behavior is what we want, though (and is consistent with
             FLambda 1): If there's a free depth variable, that means this is an
             internal recursive call, which means we consider unrolling if
             [@unrolled] appears. If it's known that the unrolling depth is
             zero, that means we're inlining into another function and we're
             done unrolling, so we immediately stop inlining.

             So this seems to be working for the moment, but I wonder what are
             the ramifications of treating unknown-ness as an observable
             property this way. Are we relying on monotonicity somewhere? *)
          ...
          then Max_inlining_depth_exceeded
            ...
            | Default_inlined ->
              ...
              in
              (* XXX if Simplify_rec_info_expr.depth_may_be_at_least dacc
                 rec_info max_rec_depth then Recursion_depth_exceeded else *)
  ...
  in
  Format.eprintf "decision: %a\n%!" Call_site_inlining_decision_type.print
    decision;
  decision
