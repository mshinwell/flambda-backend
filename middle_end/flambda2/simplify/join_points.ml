  ...
  let env, denv =
    ...
    | (_, _, (Inlinable | Non_inlinable _)) :: _ ->
      ...
      (* ( T.cut_and_n_way_join typing_env_at_fork use_envs_with_ids' ~params (*
         ...
         probably be slightly more efficient, as it wouldn't need to look at the
         middle of the three return values from Scope.Map.Split. *)
         ~unknown_if_defined_at_or_later_than: (Scope.next
         definition_scope_level) ~extra_lifted_consts_in_use_envs
         ~extra_allowed_names, denv ) *)
    | [] -> assert false
    (* see below *)
  in
  let handler_env =
