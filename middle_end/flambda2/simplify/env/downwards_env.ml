...
let define_variable (typing_env, t0) var kind =
  let typing_env =
    TE.add_variable_definition typing_env (Bound_var.var var) kind
...
let add_variable0 typing_env t0 var ty ~at_unit_toplevel =
  let typing_env =
    TE.add_equation
      (TE.add_variable_definition typing_env (Bound_var.var var) (T.kind ty)
         (Bound_var.name_mode var))
...
let define_symbol (typing_env, t0) sym ty =
  (match ty with
  | None -> ()
  | Some ty ->
  ...
  let typing_env = TE.add_symbol_definition typing_env sym ty in
  typing_env, t0
...
let add_symbol ((typing_env, t0) as t) sym ty =
  if Name_occurrences.no_variables (T.free_names ty)
  then define_symbol t sym (Some ty)
  else
    let typing_env =
      TE.add_equation
        (TE.add_symbol_definition typing_env sym None)
        (Name.symbol sym) ty
    in
    typing_env, t0
