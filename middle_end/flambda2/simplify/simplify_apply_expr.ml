...
let simplify_function_call_where_callee's_type_unavailable dacc apply
  ...
  let call_kind =
    ...
    | Direct code_id ->
      (* Some types were lost. Normally it is better to keep a direct call
         whenever we can, but stubs are simplified in environments that do not
         contain types for their closure, so the direct call generated by
         Lambda_to_flambda cannot be refined into its simplified version. In
         this case it is better to make the call indirect, because if it occurs
         at runtime we will call a simplified function instead of the original,
         unsimplified one. This also avoids having to keep old code around.
         Knowing the arity still helps though, so we use the known arity version
         of the indirect call. *)
      if DE.is_in_stub denv
