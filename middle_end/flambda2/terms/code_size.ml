...
let binary_prim_size prim =
  ...
  | Array_load (_kind, load_kind, _index_kind, _mut) ->
    (* CR mshinwell: could potentially take account of [index_kind] (same for
       [Array_set] below) *)
    array_load load_kind
...
let ternary_prim_size prim =
  ...
  | Array_set (_kind, set_kind, _index_kind) -> array_set set_kind
