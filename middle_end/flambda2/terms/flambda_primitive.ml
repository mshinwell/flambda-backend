...
let nullary_primitive_eligible_for_cse = function
  ...
  | Enter_inlined_apply _ | Begin_uninterruptable ->
    false
...
let compare_nullary_primitive p1 p2 =
  ...
  | Begin_uninterruptable, Begin_uninterruptable -> 0
  | ( Invalid _,
  ...
  | ( Probe_is_enabled _,
      (Begin_region | Enter_inlined_apply _ | Begin_uninterruptable) ) ->
    -1
  | Probe_is_enabled _, (Invalid _ | Optimised_out _) -> 1
  | Begin_region, (Enter_inlined_apply _ | Begin_uninterruptable) -> -1
  | Begin_region, (Invalid _ | Optimised_out _ | Probe_is_enabled _) -> 1
  | ( Enter_inlined_apply _,
      (Invalid _ | Optimised_out _ | Probe_is_enabled _ | Begin_region) ) ->
    1
  | Enter_inlined_apply _, Begin_uninterruptable -> -1
  | ( Begin_uninterruptable,
      ( Invalid _ | Optimised_out _ | Probe_is_enabled _ | Begin_region
      | Enter_inlined_apply _ ) ) ->
    1
...
let print_nullary_primitive ppf p =
  ...
  | Begin_uninterruptable -> Format.pp_print_string ppf "Begin_uninterruptable"
...
let result_kind_of_nullary_primitive p : result_kind =
  ...
  | Begin_uninterruptable -> Singleton K.value
...
let effects_and_coeffects_of_nullary_primitive p : Effects_and_coeffects.t =
  ...
  | Enter_inlined_apply _ ->
    ...
    Arbitrary_effects, Has_coeffects, Strict
  | Begin_uninterruptable -> Arbitrary_effects, Has_coeffects, Strict
...
let unary_primitive_eligible_for_cse p ~arg =
  ...
  | Begin_try_region | End_region | Obj_dup | End_uninterruptable -> false
...
let compare_unary_primitive p1 p2 =
  ...
  | ( ( Duplicate_array _ | Duplicate_block _ | Is_int _ | Get_tag
      ...
      | Begin_try_region | End_region | Obj_dup | End_uninterruptable ),
      _ ) ->
...
let print_unary_primitive ppf p =
  ...
  | End_uninterruptable -> Format.pp_print_string ppf "End_uninterruptable"
...
let result_kind_of_unary_primitive p : result_kind =
  ...
  | End_uninterruptable -> Singleton K.value
...
let effects_and_coeffects_of_unary_primitive p : Effects_and_coeffects.t =
  ...
  | Obj_dup ->
    ( Only_generative_effects Mutable (* Mutable is conservative *),
      Has_coeffects,
      Strict )
  | End_uninterruptable -> Arbitrary_effects, Has_coeffects, Strict
...
let unary_classify_for_printing p =
  ...
  | Begin_try_region | End_region | End_uninterruptable -> Neither
