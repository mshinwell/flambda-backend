...
module Precompute_array_size : sig
  ...
  let safe_mul_pos_vals ~loc x y =
    ...
    let product =
      Let_binding.make (Immutable Alias) (Lambda.layout_value Pintval) "product" (x * y)
    in
  ...
  let safe_product_pos ?(variable_name = "x") ~loc factors =
    let factors =
      List.map (Let_binding.make (Immutable Strict) (Lambda.layout_value Pintval)
  ...
  module Fixed_size_array = struct
    ...
    let size_nonempty ~loc = function
      | Range { start; stop; direction } ->
          ...
          let range_size =
            Let_binding.make (Immutable Alias) (Lambda.layout_value Pintval)
...
let iterator ~transl_exp ~scopes ~loc
  ...
  | Texp_comp_range { ident; pattern = _; start; stop; direction } ->
      let bound name value =
        Let_binding.make (Immutable Strict) (Lambda.layout_value Pintval)
...
let clause ~transl_exp ~scopes ~loc = function
  ...
  | Texp_comp_when cond ->
      fun body -> Lifthenelse(transl_exp ~scopes cond,
                    body,
                    lambda_unit,
                    (Lambda.layout_value Pintval) (* [unit] is immediate *))
...
let clauses ~transl_exp ~scopes ~loc = function
  ...
  | clauses ->
      let array_size =
        Let_binding.make Mutable (Lambda.layout_value Pintval)
...
let initial_array ~loc ~array_kind ~array_size ~array_sizing =
  ...
  in
  Let_binding.make array_let_kind (Lambda.layout_value Pgenval) "array" array_value
  ...
  *)
  ...
  let set_element_known_kind_in_bounds = match array_kind with
    | Pgenarray ->
        ...
        let elt = Let_binding.make (Immutable Strict) (Lambda.layout_value Pgenval) "elt" body in
...
let comprehension
  ...
  in
  let index = Let_binding.make Mutable (Lambda.layout_value Pintval) "index" (int 0) in
  ...
  | Fixed_size_info var_bindings ->
      Let_binding.let_all
        ...
        (Lifthenelse(Iterator_bindings.Fixed_size_array.are_any_empty
           ...
           (Lambda.layout_value (Parrayval array_kind))))
  | Dynamic_size_info ->
      comprehension
