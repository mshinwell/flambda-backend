...
let lookup_primitive loc ~poly_mode ~poly_sort pos p =
  ...
  let prim = match p.prim_name with
    ...
    | "%array_safe_get" ->
      Primitive
        ((Parrayrefs (gen_array_ref_kind mode, Ptagged_int_index, 1)), 2)
    | "%array_safe_set" ->
      Primitive
        (Parraysets (gen_array_set_kind (get_first_arg_mode ()), Ptagged_int_index, 1),
         3)
    | "%array_unsafe_get" ->
      Primitive
        (Parrayrefu (gen_array_ref_kind mode, Ptagged_int_index, 1), 2)
    | "%array_unsafe_set" ->
      Primitive
        ((Parraysetu (gen_array_set_kind (get_first_arg_mode ()), Ptagged_int_index, 1)),
        3)
    | "%array_safe_get_indexed_by_int64#" ->
      Primitive
        ((Parrayrefs (gen_array_ref_kind mode, Punboxed_int_index Pint64, 1)), 2)
    | "%array_safe_set_indexed_by_int64#" ->
      Primitive
        (Parraysets
          (gen_array_set_kind (get_first_arg_mode ()), Punboxed_int_index Pint64, 1),
         3)
    | "%array_unsafe_get_indexed_by_int64#" ->
      Primitive
        (Parrayrefu (gen_array_ref_kind mode, Punboxed_int_index Pint64, 1), 2)
    ...
    | "%array_safe_get_indexed_by_int32#" ->
      Primitive
        ((Parrayrefs (gen_array_ref_kind mode, Punboxed_int_index Pint32, 1)), 2)
    | "%array_safe_set_indexed_by_int32#" ->
      Primitive
        (Parraysets
          (gen_array_set_kind (get_first_arg_mode ()), Punboxed_int_index Pint32, 1),
         3)
    | "%array_unsafe_get_indexed_by_int32#" ->
      Primitive
        (Parrayrefu (gen_array_ref_kind mode, Punboxed_int_index Pint32, 1), 2)
    ...
    | "%array_safe_get_indexed_by_nativeint#" ->
      Primitive
        ((Parrayrefs (gen_array_ref_kind mode, Punboxed_int_index Pnativeint, 1)), 2)
    | "%array_safe_set_indexed_by_nativeint#" ->
      Primitive
        (Parraysets
          (gen_array_set_kind (get_first_arg_mode ()), Punboxed_int_index Pnativeint, 1),
         3)
    | "%array_unsafe_get_indexed_by_nativeint#" ->
      Primitive
        (Parrayrefu (gen_array_ref_kind mode, Punboxed_int_index Pnativeint, 1), 2)
    ...
    | "%obj_field" -> Primitive ((Parrayrefu (Pgenarray_ref mode, Ptagged_int_index, 1)), 2)
    | "%obj_set_field" ->
      Primitive
        ((Parraysetu (Pgenarray_set (get_first_arg_mode ()), Ptagged_int_index, 1)), 3)
    | "%floatarray_length" -> Primitive ((Parraylength Pfloatarray), 1)
    | "%floatarray_safe_get" ->
      Primitive ((Parrayrefs (Pfloatarray_ref mode, Ptagged_int_index, 1)), 2)
    | "%floatarray_safe_set" ->
      Primitive (Parraysets (Pfloatarray_set, Ptagged_int_index, 1), 3)
    | "%floatarray_unsafe_get" ->
      Primitive ((Parrayrefu (Pfloatarray_ref mode, Ptagged_int_index, 1)), 2)
    | "%floatarray_unsafe_set" ->
      Primitive ((Parraysetu (Pfloatarray_set, Ptagged_int_index, 1)), 3)
...
let specialize_primitive env loc ty ~has_constant_constructor prim =
  ...
  | Primitive (Parrayrefu (rt, index_kind, array_pat), arity), p1 :: _ -> begin
      let loc = to_location loc in
      let array_ref_type =
        glb_array_ref_type loc rt (array_type_kind ~elt_sort:None env loc p1)
      in
      if rt = array_ref_type then None
      else Some (Primitive (Parrayrefu (array_ref_type, index_kind, array_pat), arity))
    end
  | Primitive (Parraysetu (st, index_kind, array_pat), arity), p1 :: _ -> begin
      let loc = to_location loc in
      let array_set_type =
        glb_array_set_type loc st (array_type_kind ~elt_sort:None env loc p1)
      in
      if st = array_set_type then None
      else Some (Primitive (Parraysetu (array_set_type, index_kind, array_pat), arity))
    end
  | Primitive (Parrayrefs (rt, index_kind, array_pat), arity), p1 :: _ -> begin
      let loc = to_location loc in
      let array_ref_type =
        glb_array_ref_type loc rt (array_type_kind ~elt_sort:None env loc p1)
      in
      if rt = array_ref_type then None
      else Some (Primitive (Parrayrefs (array_ref_type, index_kind, array_pat), arity))
    end
  | Primitive (Parraysets (st, index_kind, array_pat), arity), p1 :: _ -> begin
      let loc = to_location loc in
      let array_set_type =
        glb_array_set_type loc st (array_type_kind ~elt_sort:None env loc p1)
      in
      if st = array_set_type then None
      else Some (Primitive (Parraysets (array_set_type, index_kind, array_pat), arity))
    end
