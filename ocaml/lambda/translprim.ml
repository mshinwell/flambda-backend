...
let lookup_primitive loc poly pos p =
  ...
  let prim = match p.prim_name with
    ...
    | "%begin_uninterruptable" -> Primitive (Pbegin_uninterruptable, 1)
    | "%end_uninterruptable" -> Primitive (Pend_uninterruptable, 1)
...
let lambda_primitive_needs_event_after = function
  ...
  | Pbegin_uninterruptable | Pend_uninterruptable -> false
