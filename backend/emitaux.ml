...
module Dwarf_helpers = struct
  ...
  let init ~ppf_dump ~disable_dwarf ~sourcefile =
    ...
    then Dwarf_flags.set_debug_thing Dwarf_flags.Debug_source_lines;
    match
      ( can_emit_dwarf,
  ...
  let record_dwarf_for_line_number_matrix_row ~instr_address ~file_num ~line
      ~col ~discriminator =
    match !dwarf with
    | None -> ()
    | Some dwarf ->
      Dwarf.dwarf_for_line_number_matrix_row dwarf ~instr_address ~file_num
        ~line ~col ~discriminator

  let checkpoint () =
    match !dwarf with None -> () | Some dwarf -> Dwarf.checkpoint dwarf

  let rollback () =
    match !dwarf with None -> () | Some dwarf -> Dwarf.rollback dwarf
end
