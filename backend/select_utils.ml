...
module Select_utils = struct
  ...
  let insert_op env op rs rd = self#insert_op_debug env op Debuginfo.none rs rd
  ...
  (* Emit an expression.
     ...
     Returns: - [None] if the expression does not finish normally (e.g. raises)
     - [Some rs] if the expression yields a result in registers [rs] *)
  let emit_expr (env : 'env environment) exp ~bound_name =
    self#emit_expr_aux env exp ~bound_name

  (* Emit an expression which may end some regions early.

     Returns: - [None] if the expression does not finish normally (e.g. raises)
     - [Some (rs, unclosed)] if the expression yields a result in [rs], having
     left [unclosed] (a suffix of env.regions) regions open *)
  let emit_expr_aux (env : 'env environment) exp ~bound_name :
      Reg.t array option =
    ...
    | Cconst_int (n, _dbg) ->
      ...
      ret (self#insert_op env (self#make_const_int (Nativeint.of_int n)) [||] r)
    ...
    | Ctuple exp_list -> (
      ...
      | None -> None
      | Some (simple_list, ext_env) -> ret (self#emit_tuple ext_env simple_list)
      )
