...
module DSL : sig
  ...
  val symbol :
    ?offset:int -> ?reloc:'w Arm64_ast.Symbol.same_section_or_reloc -> S.t -> 'a
  ...
  let symbol ?offset:_ ?reloc:_ _s = assert false
  ...
  let addressing addr r =
    ...
    | Ibased (s, ofs) ->
      ...
      (* see selection.ml *)
      emit_mem_symbol r ~reloc:(Different_section LOWER_TWELVE) (S.create s)
        ~offset:ofs
...
(* (* Given a SIMD operand shape and a Linear instruction, check that the Reg.t
   ...
   -> Linear.instruction -> a Arm64_ast.many = fun behaviour i -> let module
   Lane_index = Arm64_ast.Neon_reg_name.Lane_index in (* CR mshinwell: we should
   sort these cases, but into which order? *) match behaviour with | V2S_V2S_V2S
   -> let op0 = DSL.reg_v2s i.res.(0) in let op1 = DSL.reg_v2s i.arg.(0) in let
   op2 = DSL.reg_v2s i.arg.(1) in op0, op1, op2 | V4S_V4S_V4S -> let op0 =
   DSL.reg_v4s i.res.(0) in let op1 = DSL.reg_v4s i.arg.(0) in let op2 =
   DSL.reg_v4s i.arg.(1) in op0, op1, op2 | V2D_V2D_V2D -> let op0 = DSL.reg_v2d
   i.res.(0) in let op1 = DSL.reg_v2d i.arg.(0) in let op2 = DSL.reg_v2d
   i.arg.(1) in op0, op1, op2 | V16B_V16B_V16B -> let op0 = DSL.reg_v16b
   i.res.(0) in let op1 = DSL.reg_v16b i.arg.(0) in let op2 = DSL.reg_v16b
   i.arg.(1) in op0, op1, op2 | V8H_V8H_V8H -> let op0 = DSL.reg_v8h i.res.(0)
   in let op1 = DSL.reg_v8h i.arg.(0) in let op2 = DSL.reg_v8h i.arg.(1) in op0,
   op1, op2 | V4S_V8H_V8H -> let op0 = DSL.reg_v4s i.res.(0) in let op1 =
   DSL.reg_v8h i.arg.(0) in let op2 = DSL.reg_v8h i.arg.(1) in op0, op1, op2 |
   V4S_V4H_V4H -> let op0 = DSL.reg_v4s i.res.(0) in let op1 = DSL.reg_v4h
   i.arg.(0) in let op2 = DSL.reg_v4h i.arg.(1) in op0, op1, op2 | V4S_V4S ->
   let op0 = DSL.reg_v4s i.res.(0) in let op1 = DSL.reg_v4s i.arg.(0) in op0,
   op1 | V2D_V2S -> let op0 = DSL.reg_v2d i.res.(0) in let op1 = DSL.reg_v2s
   i.arg.(0) in op0, op1 | V2S_V2D_cvt -> let op0 = DSL.reg_v2s i.res.(0) in let
   op1 = DSL.reg_v2d i.arg.(0) in op0, op1 | V16B_V16B -> let op0 = DSL.reg_v16b
   i.res.(0) in let op1 = DSL.reg_v16b i.arg.(0) in op0, op1 | V2D_V2D -> let
   op0 = DSL.reg_v2d i.res.(0) in let op1 = DSL.reg_v2d i.arg.(0) in op0, op1 |
   V2D_V2D_imm6 n -> let op0 = DSL.reg_v2d i.res.(0) in let op1 = DSL.reg_v2d
   i.arg.(0) in let op2 = Arm64_ast.DSL.imm_six n in op0, op1, op2 |
   V4S_V4S_imm6 n -> let op0 = DSL.reg_v4s i.res.(0) in let op1 = DSL.reg_v4s
   i.arg.(0) in let op2 = Arm64_ast.DSL.imm_six n in op0, op1, op2 |
   V8H_V8H_imm6 n -> let op0 = DSL.reg_v8h i.res.(0) in let op1 = DSL.reg_v8h
   i.arg.(0) in let op2 = Arm64_ast.DSL.imm_six n in op0, op1, op2 |
   V16B_V16B_imm6 n -> let op0 = DSL.reg_v16b i.res.(0) in let op1 =
   DSL.reg_v16b i.arg.(0) in let op2 = Arm64_ast.DSL.imm_six n in op0, op1, op2
   | V16B_V16B_V16B_imm6 n -> let op0 = DSL.reg_v16b i.res.(0) in let op1 =
   DSL.reg_v16b i.arg.(0) in let op2 = DSL.reg_v16b i.arg.(1) in let op3 =
   Arm64_ast.DSL.imm_six n in op0, op1, op2, op3 | V8H_V8H -> let op0 =
   DSL.reg_v8h i.res.(0) in let op1 = DSL.reg_v8h i.arg.(0) in op0, op1 |
   V2S_V2D_narrow -> let op0 = DSL.reg_v2s i.res.(0) in let op1 = DSL.reg_v2d
   i.arg.(0) in op0, op1 | V4H_V4S -> let op0 = DSL.reg_v4h i.res.(0) in let op1
   = DSL.reg_v4s i.arg.(0) in op0, op1 | V8B_V8H -> let op0 = DSL.reg_v8b
   i.res.(0) in let op1 = DSL.reg_v8h i.arg.(0) in op0, op1 | V4S_V4H -> let op0
   = DSL.reg_v4s i.res.(0) in let op1 = DSL.reg_v4h i.arg.(0) in op0, op1 |
   V8H_V8B -> let op0 = DSL.reg_v8h i.res.(0) in let op1 = DSL.reg_v8b i.arg.(0)
   in op0, op1 | V4S_V2D -> let op0 = DSL.reg_v4s i.res.(0) in let op1 =
   DSL.reg_v2d i.arg.(1) in op0, op1 | V8H_V4S -> let op0 = DSL.reg_v8h
   i.res.(0) in let op1 = DSL.reg_v4s i.arg.(1) in op0, op1 | V16B_V8H -> let
   op0 = DSL.reg_v16b i.res.(0) in let op1 = DSL.reg_v8h i.arg.(1) in op0, op1 |
   RegD_RegD_RegD -> let op0 = DSL.reg_d i.res.(0) in let op1 = DSL.reg_d
   i.arg.(0) in let op2 = DSL.reg_d i.arg.(1) in op0, op1, op2 | RegS_RegS_RegS
   -> let op0 = DSL.reg_s i.res.(0) in let op1 = DSL.reg_s i.arg.(0) in let op2
   = DSL.reg_s i.arg.(1) in op0, op1, op2 | RegD_RegD -> let op0 = DSL.reg_d
   i.res.(0) in let op1 = DSL.reg_d i.arg.(0) in op0, op1 | RegS_RegS -> let op0
   = DSL.reg_s i.res.(0) in let op1 = DSL.reg_s i.arg.(0) in op0, op1 |
   RegD_RegX -> let op0 = DSL.reg_d i.res.(0) in let op1 = DSL.reg_x i.arg.(0)
   in op0, op1 | RegS_RegX -> let op0 = DSL.reg_s i.res.(0) in let op1 =
   DSL.reg_x i.arg.(0) in op0, op1 | RegX_RegD -> let op0 = DSL.reg_x i.res.(0)
   in let op1 = DSL.reg_d i.arg.(0) in op0, op1 | RegX_RegS -> let op0 =
   DSL.reg_x i.res.(0) in let op1 = DSL.reg_s i.arg.(0) in op0, op1 | Reg_LaneB
   lane -> let op0 = DSL.reg_w i.res.(0) in let op1 = DSL.reglane_b i.arg.(0)
   ~lane in op0, op1 | Reg_LaneH lane -> let op0 = DSL.reg_w i.res.(0) in let
   op1 = DSL.reglane_h i.arg.(0) ~lane in op0, op1 | Reg_LaneS lane -> let op0 =
   DSL.reg_w i.res.(0) in let op1 = DSL.reglane_s i.arg.(0) ~lane in op0, op1 |
   Reg_LaneD lane -> let op0 = DSL.reg_x i.res.(0) in let op1 = DSL.reglane_d
   i.arg.(0) ~lane in op0, op1 | RegX_V16B lane -> let lane_idx =
   Lane_index.create lane in let op0 = DSL.reg_x i.res.(0) in let op1 =
   DSL.reg_v16b i.arg.(0) in lane_idx, op0, op1 | RegX_V8H lane -> let lane_idx
   = Lane_index.create lane in let op0 = DSL.reg_x i.res.(0) in let op1 =
...
let emit_load_symbol_addr dst s =
  ...
  if macosx
  then (
    A.ins2 ADRP (DSL.reg_x dst, DSL.symbol s ~reloc:(Different_section GOT_PAGE));
    A.ins2 LDR
      ( DSL.reg_x dst,
        DSL.emit_mem_symbol dst ~reloc:(Different_section GOT_PAGE_OFF) s ))
  else if not !Clflags.dlcode
  then (
    A.ins2 ADRP (DSL.reg_x dst, DSL.symbol s);
    A.ins4 ADD_immediate
      ( DSL.reg_x dst,
        DSL.reg_x dst,
    ...
    A.ins2 ADRP (DSL.reg_x dst, DSL.symbol ~reloc:(Different_section GOT) s);
    A.ins2 LDR
      ( DSL.reg_x dst,
...
let emit_load_literal dst lbl =
  ...
  then (
    A.ins2 ADRP
      (DSL.reg_x reg_tmp1, DSL.label ~reloc:(Different_section PAGE) lbl);
    match dst.typ with
    | Float ->
      A.ins2 LDR_simd_and_fp
        ( DSL.reg_d dst,
          DSL.emit_mem_label reg_tmp1 ~reloc:(Different_section PAGE_OFF) lbl )
    | Float32 ->
      A.ins2 LDR_simd_and_fp
        ( DSL.reg_s dst,
          DSL.emit_mem_label reg_tmp1 ~reloc:(Different_section PAGE_OFF) lbl )
    ...
    A.ins2 ADRP (DSL.reg_x reg_tmp1, DSL.label lbl);
    match dst.typ with
    | Float ->
      A.ins2 LDR_simd_and_fp
        ( DSL.reg_d dst,
          DSL.emit_mem_label reg_tmp1 ~reloc:(Different_section LOWER_TWELVE)
            lbl )
    | Float32 ->
      A.ins2 LDR_simd_and_fp
        ( DSL.reg_s dst,
  ...
  | Lop (Load { memory_chunk; addressing_mode; is_atomic; _ }) -> (
    ...
    | Onetwentyeight_unaligned ->
      ...
      | Ibased (s, offset) ->
        ...
        A.ins2 ADRP
          ( DSL.reg_x reg_tmp1,
            DSL.symbol ~reloc:(Different_section PAGE) ~offset s );
        A.ins4 ADD_immediate
          ( DSL.reg_x reg_tmp1,
            DSL.reg_x reg_tmp1,
