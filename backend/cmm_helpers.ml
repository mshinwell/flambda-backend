...
(* Detect an empty for-loop and eliminate it. The pattern is: (catch rec (exit
   <lbl> <initial_arg>) with(<lbl> counter: int) (let next (+ counter 1) ; or (-
   counter 1) (if (<cmp> next stop) (exit <lbl> next) <result>))) where <result>
   does not reference counter or next. *)
