...
struct
  ...
  and effects_of (expr : Cmm.expression) =
    ...
    | Effects_of_all_expressions exprs ->
      Select_utils.Effect_and_coeffect.join_list_map args effects_of
  ...
  (* Return an array of fresh registers of the given type. Normally implemented
     as Reg.createv, but some ports (e.g. Arm) can override this definition to
     store float values in pairs of integer registers. *)

  let regs_for tys = Reg.createv tys

  let insert_moves env src dst =
    for i = 0 to min (Array.length src) (Array.length dst) - 1 do
      insert_move env src.(i) dst.(i)
    done

  (* Insert moves and stack offsets for function arguments and results *)

  let insert_move_args env arg loc stacksize =
    if stacksize <> 0
    then insert env (self#make_stack_offset stacksize) [||] [||];
    insert_moves env arg loc

  let insert_move_results env loc res stacksize =
    insert_moves env loc res;
    if stacksize <> 0
    then insert env (self#make_stack_offset (-stacksize)) [||] [||]

  (* Add an Iop opcode. Can be overridden by processor description to insert
     moves before and after the operation, i.e. for two-address instructions, or
     instructions using dedicated registers. *)

  let insert_op_debug env op dbg rs rd =
    insert_debug env (self#lift_op op) dbg rs rd;
    rd

  let insert_op env op rs rd = insert_op_debug env op Debuginfo.none rs rd

  let bind_let (env : 'env environment) v r1 =
    let env =
    ...
    then
      ...
      in
      insert_debug env naming_op Debuginfo.none [||] [||]);
    env

  let bind_let_mut (env : 'env environment) v k r1 =
  ...
  let emit_parts (env : 'env environment) ~effects_after exp =
    let module EC = Effect_and_coeffect in
      ...
      | Some r ->
        ...
        then Some (Ctuple [], env)
          ...
          then
            ...
            insert_moves env r tmp;
            Some (Cvar id, env_add (VP.create id) tmp env)

  let emit_parts_list (env : 'env environment) exp_list =
    let module EC = Effect_and_coeffect in
  ...
  let emit_tuple_not_flattened env exp_list =
  ...
  let emit_tuple env exp_list =
    Array.concat (self#emit_tuple_not_flattened env exp_list)

  let emit_extcall_args env ty_args args =
    ...
    if stack_ofs <> 0
    then insert env (self#make_stack_offset stack_ofs) [||] [||];
    List.iteri
      (fun i arg -> insert_move_extcall_arg env ty_args.(i) arg locs.(i))
      args;
  ...
  let emit_stores env dbg args regs_addr =
    let addressing_mode =
    ...
    let base =
      ...
      ref regs_addr
    in
    let for_one_arg addressing_mode arg =
      let arg =
      ...
      | Some regs -> (
            ...
            let addressing_mode =
              ...
              | Out_of_range ->
                ...
                insert_debug env
                  ...
                  dbg [||] tmp;
                insert_debug env (Op (Operation.Intop Iadd)) dbg
        ...
        | Some op ->
          insert_debug env (Op op) dbg (Array.append regs regs_addr) [||];
  ...
  let rec emit_expr env exp ~bound_name : Reg.t array option =
    (* Normal case of returning a value: no regions are closed *)
    let ret res = Some res in
    match exp with
    | Cconst_int (n, _dbg) ->
      let r = regs_for typ_int in
      ret (insert_op env (make_const_int (Nativeint.of_int n)) [||] r)
    | Cconst_natint (n, _dbg) ->
      let r = regs_for typ_int in
      ret (insert_op env (make_const_int n) [||] r)
    | Cconst_float32 (n, _dbg) ->
      let r = regs_for typ_float32 in
      ret (insert_op env (make_const_float32 (Int32.bits_of_float n)) [||] r)
    | Cconst_float (n, _dbg) ->
      let r = regs_for typ_float in
      ret (insert_op env (make_const_float (Int64.bits_of_float n)) [||] r)
    | Cconst_vec128 (bits, _dbg) ->
      let r = regs_for typ_vec128 in
      ret (insert_op env (make_const_vec128 bits) [||] r)
    | Cconst_symbol (n, _dbg) ->
      ...
      let r = regs_for typ_int in
      ret (insert_op env (make_const_symbol n) [||] r)
    | Cvar v -> (
    ...
    | Clet (v, e1, e2) -> (
      match emit_expr env e1 ~bound_name:(Some v) with
      | None -> None
      | Some r1 -> emit_expr_aux (bind_let env v r1) e2 ~bound_name)
    | Cphantom_let (_var, _defining_expr, body) ->
      emit_expr_aux env body ~bound_name
    | Ctuple [] -> ret [||]
    | Ctuple exp_list -> (
      match emit_parts_list env exp_list with
      | None -> None
      | Some (simple_list, ext_env) -> ret (emit_tuple ext_env simple_list))
    | Cop (Craise k, args, dbg) -> emit_expr_aux_raise env k args dbg
    | Cop (Copaque, args, dbg) -> (
      match emit_parts_list env args with
      | None -> None
      | Some (simple_args, env) ->
        let rs = emit_tuple env simple_args in
        ret (insert_op_debug env (make_opaque ()) dbg rs rs))
    | Cop (Ctuple_field (field, fields_layout), [arg], _dbg) -> (
      ...
      | Some loc_exp ->
        ...
        in
        ret field_slice)
    | Cop (op, args, dbg) -> emit_expr_aux_op env bound_name op args dbg
    | Csequence (e1, e2) -> (
      match emit_expr env e1 ~bound_name:None with
      | None -> None
      | Some _ -> emit_expr_aux env e2 ~bound_name)
    | Cifthenelse (econd, ifso_dbg, eif, ifnot_dbg, eelse, dbg, value_kind) ->
      emit_expr_aux_ifthenelse env bound_name econd ifso_dbg eif ifnot_dbg eelse
        dbg value_kind
    | Cswitch (esel, index, ecases, dbg, value_kind) ->
      emit_expr_aux_switch env bound_name esel index ecases dbg value_kind
    | Ccatch (_, [], e1, _) -> emit_expr_aux env e1 ~bound_name
    | Ccatch (rec_flag, handlers, body, value_kind) ->
      emit_expr_aux_catch env bound_name rec_flag handlers body value_kind
    | Cexit (lbl, args, traps) -> emit_expr_aux_exit env lbl args traps
    | Ctrywith (e1, exn_cont, v, extra_args, e2, dbg, value_kind) ->
      emit_expr_aux_trywith env bound_name e1 exn_cont v ~extra_args e2 dbg
        value_kind

  (* Emit an expression in tail position of a function, closing all regions in
     [env.regions] *)
  let emit_tail env sub_cfg exp =
    match exp with
    | Clet (v, e1, e2) -> (
      match emit_expr env sub_cfg e1 ~bound_name:None with
      | None -> ()
      | Some r1 -> emit_tail (bind_let env sub_cfg v r1) e2)
    ...
    | Csequence (e1, e2) -> (
      match emit_expr env sub_cfg e1 ~bound_name:None with
      | None -> ()
      | Some _ -> emit_tail env sub_cfg e2)
    | Cifthenelse (econd, ifso_dbg, eif, ifnot_dbg, eelse, dbg, value_kind) ->
  ...
  module State = struct
    type t = { mutable sub_cfg : Sub_cfg.t }
    (* XXX does this really need to be mutable? *)

    let create = { sub_cfg = Sub_cfg.make_empty () }
  end

  let insert_debug (_env : environment) basic dbg arg res =
    Sub_cfg.add_instruction sub_cfg basic arg res dbg

  let insert_op_debug_returning_id (_env : environment) op dbg arg res =
    let instr = Cfg.make_instr (Cfg.Op op) arg res dbg in
    Sub_cfg.add_instruction' sub_cfg instr;
    instr.id

  let insert (_env : environment) basic arg res =
    (* CR mshinwell: fix debuginfo *)
    Sub_cfg.add_instruction sub_cfg basic arg res Debuginfo.none

  let insert' (_env : environment) term arg res =
    (* CR mshinwell: fix debuginfo *)
  ...
  let insert_op_debug' (_env : environment) op dbg rs rd =
    Sub_cfg.set_terminator sub_cfg op rs rd dbg;
    rd

  let tailrec_label : Label.t = (* MUTABLE *) Label.none
  (* set in emit_fundecl *)
  ...
  let emit_expr_aux_ifthenelse env bound_name econd _ifso_dbg eif
    ...
    | Some rarg ->
      assert (Sub_cfg.exit_has_never_terminator sub_cfg);
      let rif, (sif : state) = emit_sequence t env eif ~bound_name in
      let relse, (selse : state) = emit_sequence t env eelse ~bound_name in
  ...
  let emit_expr_aux_catch env bound_name (_rec_flag : Cmm.rec_flag) handlers
  ...
  let emit_expr_aux_trywith env bound_name e1 exn_cont v ~extra_args e2
  ...
  let emit_sequence ?at_start (env : environment) exp ~bound_name : _ * state =
  ...
  let insert_return (env : environment) r (traps : trap_action list) =
    match r with
    | None -> ()
    | Some r ->
      List.iter
  ...
  let emit_tail_switch env esel index ecases (_dbg : Debuginfo.t)
    ...
    | Some rsel ->
      ...
      in
      Sub_cfg.update_exit_terminator sub_cfg term_desc ~arg:rsel;
      sub_cfg <- Sub_cfg.join_tail ~from:(Array.to_list sub_cases) ~to_:sub_cfg

  let emit_tail_catch env (_rec_flag : Cmm.rec_flag) handlers e1
      (_value_kind : Cmm.kind_for_unboxing) =
    let handlers =
      List.map
        (fun (nfail, ids, e2, dbg, is_cold) ->
  ...
  let emit_tail_trywith env e1 exn_cont v ~extra_args e2 (_dbg : Debuginfo.t)
      (_value_kind : Cmm.kind_for_unboxing) =
    (* CR-someday xclerc for xclerc: use the `_dbg` parameter *)
    assert (Sub_cfg.exit_has_never_terminator sub_cfg);
    let exn_label = Cmm.new_label () in
    ...
    | { traps_ref = { contents = Unreachable }; _ } ->
      (* Note: The following [unreachable] expression has machtype [|Int|], but
         ...
         tails we will need to implement something like the [emit_expr_aux]
         version above, that hides the machtype. *)
      let unreachable =
        Cmm.(
          Cop
            ( Cload
  ...
  let emit_tail_sequence ?at_start env exp =
    let s = {<sub_cfg = Sub_cfg.make_empty ()>} in
    (match at_start with None -> () | Some f -> f s);
    s#emit_tail env exp;
    s#extract
    ...
    List.iteri
      (fun param_index (var, _ty) ->
        ...
        then
          ...
          in
          insert_debug t env (Cfg.Op naming_op) Debuginfo.none hard_regs_for_arg
            [||])
      f.Cmm.fun_args;
