...
let select_operation (op : Cmm.operation) (args : Cmm.expression list)
    (dbg : Debuginfo.t) ~label_after : basic_or_terminator * Cmm.expression list
    =
  ...
  | Cextcall { func; builtin = true } ->
    Misc.fatal_errorf "Selection.select_operation: builtin not recognized %s"
      func ()
  | Cextcall { func; alloc; ty; ty_args; returns; builtin = false; effects; _ }
    ->
    let external_call =
  ...
  | Cpackf32 | Copaque | Cbswap _ | Cprefetch _ | Craise _ | Ctuple_field (_, _)
    ->
...
let select_arith_comp (cmp : Simple_operation.integer_comparison)
    (args : Cmm.expression list) : basic_or_terminator * Cmm.expression list =
  match args with
  | [arg; Cconst_int (n, _)] when is_immediate t (Simple_operation.Icomp cmp) n
    ->
...
let (* MUTABLE *) sub_cfg = Sub_cfg.make_empty ()
...
let emit_expr_aux_op env bound_name op args dbg =
  ...
  | Some (simple_args, env) -> (
    ...
    let new_op, new_args = select_operation t op simple_args dbg ~label_after in
    match new_op with
...
let emit_expr_aux_catch env bound_name (_rec_flag : Cmm.rec_flag) handlers body
    (_value_kind : Cmm.kind_for_unboxing) =
  let handlers =
    List.map
  ...
  let env, handlers_map =
    ...
    List.fold_left
      (fun (env, map) (nfail, ids, rs, e2, dbg, is_cold) ->
        let label = Cmm.new_label () in
        let env, r = Select_utils.env_add_static_exception nfail rs env label in
  ...
  | Some (simple_list, ext_env) -> (
    ...
    | Lbl nfail ->
      ...
      let handler =
        ...
        with Not_found ->
          Misc.fatal_error
            ("Selection.emit_expr: unbound label " ^ Stdlib.Int.to_string nfail)
      in
...
let emit_expr_aux_trywith env bound_name e1 exn_cont v ~extra_args e2
  ...
  (* For each exception handler having extra arguments, distinguished registers
     corresponding to such arguments are created. They are populated at each
     raise site, which is the only place Cmm allows extra arguments to be
     involved in a raise. Any function call that involves extra arguments on its
  ...
  let with_handler env_handler e2 =
    ...
    Sub_cfg.add_instruction_at_start s2 (Cfg.Op Move) [| Proc.loc_exn_bucket |]
...
let emit_sequence ?at_start (env : environment) exp ~bound_name : _ * 'self =
...
let emit_tail_catch env (_rec_flag : Cmm.rec_flag) handlers e1
  ...
  let rec build_all_reachable_handlers ~already_built ~not_built =
    ...
    then already_built
      ...
      build_all_reachable_handlers ~already_built ~not_built
  in
  let new_handlers : (int * Simple_operation.trap_stack * Sub_cfg.t * bool) list
      =
    build_all_reachable_handlers ~already_built:[] ~not_built:handlers_map
    (* Note: we're dropping unreachable handlers here *)
  in
...
let emit_tail_trywith env e1 exn_cont v ~extra_args e2 (_dbg : Debuginfo.t)
  ...
  | { traps_ref = { contents = Unreachable }; _ } ->
    (* Note: The following [unreachable] expression has machtype [|Int|], but
       this might not be the correct machtype for this function's return value.
       It doesn't matter at runtime since the expression cannot return, but if
       we start checking (or joining) the machtypes of the different tails we
       will need to implement something like the [emit_expr_aux] version above,
       that hides the machtype. *)
    let unreachable =
      Cmm.(
        Cop
          ( Cload
    ...
    in
    with_handler env unreachable
  (* Misc.fatal_errorf "Selection.emit_expr: \ Unreachable exception handler %d"
     lbl *)
  | exception Not_found ->
    Misc.fatal_errorf "Selection.emit_expr: Unbound handler %d" exn_cont
...
let emit_fundecl ~future_funcnames f =
  ...
  let cfg =
    ...
    Cfg.create ~fun_name:f.Cmm.fun_name.sym_name ~fun_args:loc_arg
      ~fun_codegen_options:(Cfg.of_cmm_codegen_option f.Cmm.fun_codegen_options)
  ...
  let delete_prologue_poll =
    (* CR mshinwell/xclerc: find a neater way of doing this rather than making a
       special case for the [optimistic_prologue_poll_instr_id]. *)
    not
  ...
  (* note: `Cfgize.Stack_offset_and_exn.update_cfg` may add edges to the graph,
     and should hence be executed before
