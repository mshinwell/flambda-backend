...
let create normal_or_dwo ~start_of_code_symbol ~end_of_code_symbol
  ...
  let values =
    [ (* The initial length is inserted here by the code below. *)
      ...
      (* The mystery values match up with what gcc emits and stop bfd from
         mangling the aranges tables. They do not appear to be referenced in the
         DWARF specification. *)
      ...
      (* The terminating entry is only two words since the segment selector word
         is again absent. *)
  ...
  let size =
    List.fold_left
      (fun size value -> Dwarf_int.add size (V.size value))
      (Dwarf_int.zero ()) values
  in
  { size; values }
