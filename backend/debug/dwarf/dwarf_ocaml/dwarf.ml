...
let create ~sourcefile ~unit_name ~asm_directives ~get_file_id ~code_layout =
  ...
  let debug_line_section =
    let code_begin =
      match (code_layout : DS.code_layout) with
      | Continuous_code_section { code_begin; _ } -> code_begin
      | Function_sections ->
        (* CR mshinwell: support [Function_sections] in the debug_line section
           emission. *)
        Asm_symbol.create_global "__function_sections_debug_line_placeholder"
    in
    Debug_line_section.create ~code_begin
  in
  let state =
    DS.create ~compilation_unit_header_label ~compilation_unit_proto_die
      ~code_layout ~debug_line_section debug_loc_table debug_ranges_table
