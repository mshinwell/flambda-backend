  ...
  then None
    ...
    | Reg n -> (
      let dwarf_reg_number =
        ...
        (* This [None] case isn't an error to cover situations such as found in
           the i386 backend where [num_available_registers] does not extend to
           the end of the register arrays (in that case for the x87 top of stack
           register). *)
    ...
    | Stack _ -> (
      ...
      | None ->
        Misc.fatal_errorf
          "Register %a assigned to stack but no offset from CFA provided"
          Printmach.reg reg
      | Some offset_from_cfa_in_bytes ->
        if offset_from_cfa_in_bytes mod Arch.size_addr <> 0
        then
          Misc.fatal_errorf "Misaligned stack slot at offset %d (reg %a)"
            offset_from_cfa_in_bytes Printmach.reg reg;
...
let offset_from_cfa_in_bytes reg stack_loc ~stack_offset ~fun_contains_calls
    ~fun_num_stack_slots =
  let frame_size =
    Proc.frame_size ~stack_offset ~fun_contains_calls ~fun_num_stack_slots
  in
  let slot_offset =
