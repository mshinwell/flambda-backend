...
(* Old code: val create : compilation_unit_header_label:Asm_label.t ->
   compilation_unit_proto_die:Proto_die.t -> value_type_proto_die:Proto_die.t ->
   naked_float_type_proto_die:Proto_die.t -> start_of_code_symbol:Asm_symbol.t
   -> end_of_code_symbol:Asm_symbol.t -> Address_table.t -> Debug_loc_table.t ->
   Debug_ranges_table.t -> Location_list_table.t -> Range_list_table.t ->
   Dwarf_version.t -> t
   ...
   val function_abstract_instances : t -> (Proto_die.t * Asm_symbol.t)
   Debuginfo.Function.Id.Tbl.t

   val die_symbols_for_external_declarations : t -> Asm_symbol.t
   Asm_symbol.Tbl.t
   ...
   val record_type_die_for_lifted_constant : t -> Asm_symbol.t -> Proto_die.t ->
   unit

   val type_die_for_lifted_constant : t -> Asm_symbol.t -> Proto_die.t option

   val find_die_for_module_path : t -> module_path:Path.t -> Proto_die.t option

   val record_die_for_module_path : t -> module_path:Path.t -> Proto_die.t ->
   unit *)
