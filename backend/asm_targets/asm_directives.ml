...
let debug_header ~get_file_num =
  ...
  (* Stop dsymutil complaining about empty __debug_line sections (produces bogus
     ...
     never empty. The following line is behind a flag because it adds a loc
     directive in a section which is not the .text section, which causes issues
     when the debug_line section is being created by the internal assembler. *)
