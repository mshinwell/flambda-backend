...
caml_result caml_do_pending_actions_res(void)
{
  ...
  if (caml_result_is_exception(result)) {
    check_async_exn(Result_as_exception(result), "signal handler");
    goto exception;
  }
  ...
  if (caml_result_is_exception(result)) {
    check_async_exn(Result_as_exception(result), "memprof callback");
    goto exception;
  }
  ...
  if (caml_result_is_exception(result)) {
    check_async_exn(Result_as_exception(result), "finaliser");
    goto exception;
  }
...
{
  caml_result res = caml_process_pending_actions_with_root_res(root);
  if (caml_result_is_exception(res)) {
    return caml_raise_async_if_exception(Result_as_exception(res), "");
  }
  return res.data;
}
...
CAMLprim value caml_install_signal_handler(value signal_number, value action)
{
  ...
  caml_result res_pending = caml_process_pending_signals_res();
  if (caml_result_is_exception(res_pending)) {
    (void) caml_raise_async_if_exception(Result_as_exception(res_pending), "");
  }
  CAMLreturn (res);
 err:
  caml_plat_unlock(&signal_install_mutex);
