...
(* Read the [ld.conf] file and return the corresponding list of directories.
   This OCaml-side reimplementation of [caml_dynlink_parse_ld_conf] (in
   [runtime/dynlink.c]) is needed because the boot compiler is linked against
   the host opam OCaml runtime, which does not have the new primitive. *)
