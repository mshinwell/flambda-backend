...
let make_constructor
  ...
  | Some sret_type ->
      ...
      univars, targs, Some tret_type, args, Some ret_type
      end
  ...
  *)
  ...
  (* Add abstract row *)
    ...
    let typ_shape =
      ...
      | Ttype_record lbls -> Shape.str ~uid (shape_map_labels lbls)
...
let transl_type_decl env rec_flag sdecl_list =
  ...
  let tdecls, decls, shapes, temp_env, new_env, delayed_jkind_checks =
    Ctype.with_local_level_generalize begin fun () ->
      (* Enter types. *)
      let temp_env =
      ...
      let tdecls =
        List.map2
          (fun (tdecl, _) (_, decl) -> { tdecl with typ_type = decl }) tdecls decls
      in
      current_slot := None;
      ...
      let delayed_jkind_checks =
        ...
        | Asttypes.Recursive ->
          List.map2
            ...
            ids_list sdecl_list
      in
      (tdecls, decls, shapes, temp_env, new_env, delayed_jkind_checks)
    end
  in
...
||||||| 23e84b8c4d
  ...
  | Constraint_failed (env, err) ->
      ...
      | Function_label_mismatch _ | Tuple_label_mismatch _ | First_class_module _ -> None
      in
